...
describe 'Project' do
	...
	it 'can create a new project with a name and amount' do
		new_project = Project.new(default_project_name, default_project_amount)
	...
	it 'raises an ArgumentError if a project name contains invalid characters' do
		...
		expect(Project.new('Fancy.Cooler', default_project_amount)).to raise_error(ArgumentError, invalid_character_error_message)
	end
	...
	it 'raises an ArgumentError if a project name is less than 4 characters or more than 20' do
		...
		expect(Project.new('Fan', default_project_amount)).to raise_error(ArgumentError, invalid_length_error_message)
		expect(Project.new('The_Fanciest_Of_Coolers', default_project_amount)).to raise_error(ArgumentError, invalid_length_error_message)
	end

	it 'allows both dollars and cents in the project amount' do
		project_amount = '1000.99'
		new_project = Project.new(default_project_name, project_amount)
	...
	it 'raises an ArgumentError if a project amount contains "$"' do
		...
		expect(Project.new(default_project_name, project_amount)).to raise_error(ArgumentError, invalid_amount_message)
	end
